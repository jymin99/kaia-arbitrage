...
// 스왑 함수
const swapwithExactInputMultihop = async () => {
  // 경로 생성 (BTCB -> USDC -> WETH)
  ...
  try {
    const tokenInContract=new ethers.Contract(USDC,ERC20ABI,wallet);

    const approveTx = await tokenInContract.approve(routerAddress, amountIn);
    await approveTx.wait();

    // const transferTx = await tokenInContract.transferFrom(wallet.address,routerAddress,amountIn);
    // await transferTx.wait();

    const swapTx =await routerContract.exactInput({
      path,
      recipient,
      deadline, // 10분 후
      amountIn,
      amountOutMinimum: 0, // 최소 수량 설정
    });
